...
<body class="bg-canvas text-text-primary antialiased min-h-screen font-sans">
    ...
    <aside class="canva-sidebar hidden lg:flex fixed left-0 top-0 bottom-0 w-64 flex-col p-6 z-40 border-r border-gray-100/50 bg-white/50 backdrop-blur-xl">
        <div class="mb-10">
            ...
            <span class="text-xs font-semibold text-blue-700 bg-blue-50 px-2 py-0.5 rounded-full mt-1 inline-block">DOCTOR PANEL</span>
        </div>

        <nav class="flex-1 space-y-1 overflow-y-auto">
            <a href="{{ route('doctor.dashboard') }}" class="nav-link {{ request()->routeIs('doctor.dashboard') ? 'active' : 'text-gray-500 hover:bg-gray-50' }} flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all">
                <i data-lucide="layout-dashboard" class="w-5 h-5"></i>
                <span>Dashboard</span>
            </a>
            <a href="#" class="nav-link text-gray-500 hover:bg-gray-50 flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all mt-1">
    ...
    <nav class="lg:hidden fixed bottom-0 left-0 right-0 glass border-t border-white/40 px-6 py-3 z-50">
        <div class="flex justify-around items-center">
            <a href="{{ route('doctor.dashboard') }}" class="mob-nav flex flex-col items-center gap-1 {{ request()->routeIs('doctor.dashboard') ? 'text-mint-dark' : 'text-gray-400' }}">
                <i data-lucide="layout-dashboard" class="w-6 h-6"></i>
                <span class="text-[10px] font-medium">Dashboard</span>
            </a>
            <a href="#" class="mob-nav flex flex-col items-center gap-1 text-gray-400">
                <i data-lucide="users" class="w-6 h-6"></i>
                <span class="text-[10px] font-medium">Daftar Pasien</span>
            </a>
        </div>
    </nav>
